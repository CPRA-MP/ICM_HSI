...
# 2023 Update -- SES 6/30/20 - removed ChlAdict, TSSdict and all variables as not in use for 2023
...
# 2023 Update -- SES 6/27/20 updated a good bit of brown shrimp models, removed or wrote over old code
    ...
    with open(HSIcsv,'w') as fBSS, open(HSIcsv2,'w') as fBST:
        ...
        headerstring_s = 'GridID,HSI,s,t,v2,oysc,savc\n'
        headerstring_t = 'GridID,HSI,s,t,v2\n'
        ...
        for gridID in gridIDs:
            ...
            oysc = max(0.0,min(cultchdict[gridID],1.0))    # 2023 Update -- SES 7/1/20 oyster cultch not yet set to mean oyster HSI for time period
 #           dayvs = 1.35
...
##############################################
 ...
 #   all predictor variables are converted to z-scores using mean, sd from glmms in WQ SI memo
          zscs = (ss - 7.94)/7.07
          zscss = (ss**2. - 112.91)/165.81
          zsct = (ts - 26.87)/3.73
          zsctt = (ts**2. - 735.69)/191.54
            ...
            writestring = '%s,%s,%s,%s,%s,%s,%s\n' %(gridID,HSI_BrShrSeine,ss,ts,v2s,oysc,savc)
...
#   predictor variables are converted to z-scores using mean, sd from glmms in WQ SI memo
          zscst = (st - 10.97)/8.03
          zscsst = (st**2. - 184.85)/216.00
          zsctt = (tt - 26.64)/3.73
          zscttt = (tt**2. - 723.40)/189.05
            
           lnCPUE1t = 2.68 + 1.54*zscst + 0.86*zsctt - 1.51*zscsst - 072*zscttt - 0.18*zscst*zsctt
           ...
           S1t = (e**lnCPUE1t - 1.)/24.61
            ...
            if st < 0. or st > 37.61.:          # check if salinity or temp outside of model predicted ranges from WQ SI memo and if so, set S1s=-99.99 to flag
                S1s=t = -99.99          

            if tt < 11.65 or tt > 35.21:
                S1t = -99.99
...
# delete any dictionaries that aren't used in any other HSIs - frees up memory
    del(sal_AprJul_ave,tmp_AprJul_ave)

# delete temporary variables so they do not accidentally get used in other HSIs
    del(ss,ts,dayvs,v2s,lnCPUE1s,S1s,S2s,savc,oysc,st,tt,dayvt,v2t,lnCPUE1t,S1t,S2t)
